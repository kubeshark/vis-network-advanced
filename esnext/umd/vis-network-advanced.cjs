/**
 * vis-network-advanced
 * https://visjs.github.io/vis-network-advanced/
 *
...
class PhysicsEngine {
  ...
  init() {
    ...
    if (this.options.solver === "forceAtlas2Based") {
      ...
      // if wasm usage is requested and a wasm compute function is present, use a thin JS adapter
      if (options.useWasm === true && window && window.forceatlas_compute_forces) {
        this.nodesSolver = {
          // solver interface: solve()
          solve: () => {
            const nodeIndices = this.physicsBody.physicsNodeIndices;
            const nodes = this.body.nodes;
            ...
            // build contiguous typed arrays [x0,y0,x1,y1,...]
            ...
            // call wasm function (should be exposed as window.forceatlas_compute_forces)
            // it must accept Float64Array, Float64Array, number and return Float64Array forces
            const gravitational = this.modelOptions.gravitationalConstant || 0;
            const theta = this.modelOptions.theta || 0.5;
            const avoidOverlap = this.modelOptions.avoidOverlap || 0;
            // radii: build array of node shape radii if available
            const radii = new Float64Array(n);
            for (let i = 0; i < n; i++) {
              const node = nodes[nodeIndices[i]];
              radii[i] = (node.shape && node.shape.radius) ? node.shape.radius : 0;
            }

            // degrees: pass degree (edges count + 1) to match JS FA2 implementation
            ...
            const wasmForces = window.forceatlas_compute_forces(
              positions,
              masses,
              theta,
              gravitational,
            ...
            // write results back into physicsBody.forces using original node ids
            // wasmForces expected: [fx0,fy0,fx1,fy1,...]
            for (let i = 0; i < n; i++) {
              const id = nodeIndices[i];
              this.physicsBody.forces[id].x += wasmForces[2 * i];
              this.physicsBody.forces[id].y += wasmForces[2 * i + 1];
            }
          },
        };
        ...
        if (window.forceatlas_compute_springs) {
          this.edgesSolver = {
            solve: () => {
              ...
              const wasmSprings = window.forceatlas_compute_springs(
                positions,
                fromIdx,
                toIdx,
                lengths,
                this.modelOptions.springConstant || 0,
              );
        ...
        if (window.forceatlas_compute_central_gravity) {
          this.gravitySolver = {
            solve: () => {
              ...
              const wasmGravity = window.forceatlas_compute_central_gravity(
                positions,
                masses,
                this.modelOptions.centralGravity || 0,
                degrees,
              );
...
class CanvasRenderer {
  ...
  _drawNodes(ctx, alwaysShow = false) {
    ...
    // draw hovered nodes above everything else: fixes https://github.com/kubeshark/vis-network-advanced/issues/226
...
class Canvas {
  ...
  _create() {
    ...
    this.frame.className = "vis-network-advanced";
...
//# sourceMappingURL=vis-network-advanced.cjs.map
