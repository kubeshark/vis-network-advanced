  ...
  init() {
    ...
    if (this.options.solver === "forceAtlas2Based") {
      ...
      if (
        ...
        window &&
        window.forceatlas_compute_forces
      ) {
        this.nodesSolver = {
          ...
          solve: () => {
            ...
            for (let i = 0; i < n; i++) {
              const node = nodes[nodeIndices[i]];
              radii[i] =
                node.shape && node.shape.radius ? node.shape.radius : 0;
            }
            ...
            for (let i = 0; i < n; i++) {
              const node = nodes[nodeIndices[i]];
              degrees[i] =
                node.edges && node.edges.length ? node.edges.length + 1 : 1;
            }
          ...
          this.edgesSolver = {
            solve: () => {
              ...
              for (let i = 0; i < edgeIndices.length; i++) {
                ...
                fromIdx[i] =
                  indexMap[edge.fromId] !== undefined
                    ? indexMap[edge.fromId]
                    : -1;
                toIdx[i] =
                  indexMap[edge.toId] !== undefined ? indexMap[edge.toId] : -1;
                lengths[i] =
                  edge.options.length === undefined
                    ? this.modelOptions.springLength
                    : edge.options.length;
              }
