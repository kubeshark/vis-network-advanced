 ...
 * @param {number} theta
 * @param {number} gravitational
 * @param {number} avoid_overlap
 * @param {Float64Array} radii
 * @param {Float64Array} degrees
 * @returns {Float64Array}
 */
export function compute_forces(positions, masses, theta, gravitational, avoid_overlap, radii, degrees) {
    const ret = wasm.compute_forces(positions, masses, theta, gravitational, avoid_overlap, radii, degrees);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('forceatlas_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
